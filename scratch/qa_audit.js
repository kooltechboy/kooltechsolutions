...
async function runAudit() {
  console.log("Starting E2E Homepage QA Audit Campaign...");
  ...
  // Testimonial cards
  const testimonials = await page.$$('.glass-card, [style*="border-radius"]');
  console.log(`Found testimonials or cards count: ${testimonials.length}`);
  ...
  for (const btn of scheduleBtns) {
    ...
    if (text.toLowerCase().includes('schedule') || text.toLowerCase().includes('consultation') || text.toLowerCase().includes('cita')) {
      scheduleBtn = btn;
      break;
    }
  }
  ...
  if (scheduleBtn) {
    ...
    if (nameInput && emailInput && phoneInput) {
      ...
      // Close the modal
      const closeBtn = await page.$('button:has(svg)');
      if (closeBtn) {
